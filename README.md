...
| 市场 | 代码格式 | 数据源 | 实时报价 |
| :--- | :--- | :--- | :--- |
| 美股 (US) | 1–5 位字母，如 `AAPL` | Yahoo Finance | Yahoo Finance |
| A 股 (CN) | 6 位数字，如 `600519` / `600519.SH` | Yahoo Finance（`600519.SS`） | Yahoo Finance |
| 港股 (HK) | 4–5 位数字，如 `00700.HK` | Yahoo Finance（`0700.HK`） | Yahoo Finance |
...
│                    基础设施层                             │
│  ┌──────────┐  ┌──────────────────────┐  ┌────────────┐ │
│  │PostgreSQL│  │   Yahoo Finance API   │  │ DashScope  │ │
...
> **注意：** 财务数据使用 Yahoo Finance，无需 API Key。所有密钥请通过 `.env` 文件注入，严禁硬编码到源码中。`.env` 已在 `.gitignore` 中忽略。
