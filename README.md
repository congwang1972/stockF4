...
| 市场 | 代码格式 | 数据源 | 实时报价 |
| :--- | :--- | :--- | :--- |
| 美股 (US) | 1–5 位字母，如 `AAPL` | Alpha Vantage API | 腾讯行情 |
| A 股 (CN) | 6 位数字，如 `600519` / `600519.SH` | Tushare API | 腾讯行情 |
| 港股 (HK) | 4–5 位数字，如 `00700.HK` | 暂不支持 | 腾讯行情 |
...
| 财务数据 | Alpha Vantage（美股）、Tushare（A 股）、腾讯行情 | — |
| 校验库 | Zod | 3.22 |
...
│                    基础设施层                             │
│  ┌──────────┐  ┌──────────────────────┐  ┌────────────┐ │
│  │PostgreSQL│  │ External APIs         │  │ DashScope  │ │
│  │ (Prisma) │  │Alpha Vantage/Tushare  │  │  (Qwen)    │ │
│  └──────────┘  └──────────────────────┘  └────────────┘ │
└──────────────────────────────────────────────────────────┘
```
...
获取财务数据 ──(失败)──▶ 标记 failed → 返回对应错误码
    │  ├─ US: Alpha Vantage (财务报表 + 公司概况)
    │  ├─ CN: Tushare (利润表/现金流/资产负债表/财务指标)
    │  └─ HK: 返回 MARKET_NOT_SUPPORTED
    │
    ▼
...
# 财务数据源
TUSHARE_TOKEN=your_tushare_token          # A 股数据，前往 tushare.pro 注册获取
ALPHA_VANTAGE_API_KEY=your_alpha_vantage_api_key  # 美股数据，前往 alphavantage.co 注册获取
...
> **注意：** 所有密钥请通过 `.env` 文件注入，严禁硬编码到源码中。`.env` 已在 `.gitignore` 中忽略。
